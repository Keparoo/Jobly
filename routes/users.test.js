...
describe('GET /users/:username', function() {
	...
	test('fail for other user', async function() {
		const resp = await request(app)
			.get(`/users/u1`)
...
describe('POST /users/:username/jobs/:id', function() {
	...
	test('fail auth for other users', async function() {
		const resp = await request(app)
			.post(`/users/u1/jobs/${testJobIds[1]}`)
			.set('authorization', `Bearer ${u2Token}`);
		expect(resp.statusCode).toEqual(401);
	...
	test('fail auth for anon', async function() {
		const resp = await request(app).post(`/users/u1/jobs/${testJobIds[1]}`);
		expect(resp.statusCode).toEqual(401);
	});

	test('fail: not found for no such username', async function() {
		const resp = await request(app)
	...
	test('fail: not found for no such job', async function() {
		const resp = await request(app)
	...
	test('fail: bad request invalid job id', async function() {
		const resp = await request(app)
			.post(`/users/u1/jobs/0`)
			.set('authorization', `Bearer ${adminToken}`);
		expect(resp.statusCode).toEqual(404);
